...
void FusionEKF::ProcessMeasurement(const MeasurementPackage& measurement_pack)
{
  ...
  if (!is_initialized_) {
    ...
    } else if (measurement_pack.sensor_type_ == MeasurementPackage::ELECTRONIC) {
      ekf_.x_(0) = measurement_pack.raw_measurements_(0);
      ekf_.x_(1) = measurement_pack.raw_measurements_(1);
  ...
  ekf_.Q_ << dt4 / 4 * noise_ax, 0, dt3 / 2 * noise_ax, 0, 0, dt4 / 4 * noise_ay, 0,
      dt3 / 2 * noise_ay, dt3 / 2 * noise_ax, 0, dt2 * noise_ax, 0, 0, dt3 / 2 * noise_ay, 0,
      dt2 * noise_ay;
